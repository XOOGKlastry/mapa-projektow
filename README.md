...
Punkty na mapie Polski, każdy z kartą atrybutów (nazwa, status, pakiet, miejscowość, adres, podwykonawca, zakres, notatki), do wspólnej edycji przez link. Jedna strona statyczna, bez budowania, bez kluczy API po stronie mapy. Trasy i odczyt adresów z obrazu zostały z poprzedniej wersji, przypisania handlowców zniknęły.
...
Zbiór ustawiasz w adresie ręcznie: dopisz `?zbior=nazwa` na końcu linku i podeślij go zespołowi. Pusta mapa dla nowego zbioru powstaje sama przy pierwszym wejściu pod nowym adresem.
...
```js
{ klucz: 'pakiet', etykieta: 'Pakiet', typ: 'lista', filtr: true, wKarcie: true,
  opcje: [{ wartosc: 'P1' }, { wartosc: 'P2' }] }
```
...
Zaznaczasz punkty ikoną `↝` w liście albo przyciskiem w karcie na mapie, wybierasz zakończenie trasy i klikasz *Wyznacz kolejność*. Gwiazdka przy przystanku nadaje mu priorytet: solver ustawia punkty oznaczone gwiazdką przed resztą (optymalnie między sobą), więc trafiają na początek trasy, nie na koniec. Kolejność wewnątrz grupy priorytetowej dalej liczy się najkrótsza.
...
assets/nfosigw.png    logo NFOŚiGW w rogu mapy
